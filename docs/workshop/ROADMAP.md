...
| D2.7 | Deck player, present mode | `present.html`, `src/player.js` | Deck = flatten(items→slides), keyboard nav (FR-6.1), deterministic slide+step state, per-item countdown timer w/ overrun red, schedule bar, notes drawer, overview grid (Esc). |
